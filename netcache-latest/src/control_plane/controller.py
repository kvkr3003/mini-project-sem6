...
copy_to_cpu_function_call =0
f= open("../kv_store/data/deleted.txt","w")

f2= open("../kv_store/data/inserted.txt","w")
...
f3= open("../kv_store/data/memory.txt","w")

thread_file= open("../control_plane/thread-info.txt","w")

counter=0
start_time= time.time()

def my_function(Packet):
    print("Class Packet",Packet)
...
class NCacheController(object):
    ...
    def cache_lfu_eviction(self, threshold=0.8, sampling=0.2, to_remove=0.5):
        ...
        print("entering for cache eviction",self.used_mem_slots)

        global counter

        if counter==0:
            current_time= time.time()
            time_to_write= current_time - start_time
            f3.write(str(time_to_write))
            f3.write("\n")
            counter= counter+1
    ...
    def evict(self, key):
        ...
        # f.write("register_write cache_status" + str(key_idx) )
    ...
    def recv_switch_updates(self, pkt):
        ...
        if pkt.haslayer(UDP):
            ...
            ncache_header = NetcacheHeader(pkt[UDP][Raw].load)

            # print("UDP payload is",pkt[UDP][Raw].load)
            # ncache_header = NetcacheHeader(pkt[UDP])
        ...
        global copy_to_cpu_function_call

        print("number of times hot key is reported is",copy_to_cpu_function_call)

        print("cache memory current entries are",self.used_mem_slots)

        print("cache size is",len(self.mem_pool) * VTABLE_SLOT_SIZE * 0.8)
        

        if op == NETCACHE_HOT_READ_QUERY:
            copy_to_cpu_function_call= copy_to_cpu_function_call + 1
            # print("packet is",pkt)
            # if pkt.haslayer(UDP):
                # print("yes")
                # print("UDP payload is",pkt[UDP].payload)
                # temp_header= NetcacheHeader(pkt[UDP].payload)
                # print("netchace header is", temp_header)
            # print("header is ",ncache_header)
            ...
            # key=key[2:]
            # value=value[6:]
            # print("key-value pair to insert is",key,value)
            self.insert(key, value)
    ...
    def main(self):
        ...
        # thread_file.write("\n")
        # thread_file.write(str(duration))
        # thread_file.write("\n")
        # thread_file.write("end1")


def get_threads_cpu_percent(p, interval=1):
   total_percent = p.cpu_percent(interval)
   total_time = sum(p.cpu_times())
   print(total_percent,total_time)
...
def func2():
    ...
    thread_file.write(str(duration))
    thread_file.write("\n")
    thread_file.write("end2")

    # end_time= time.time()-start_time
    # thread_file.write(str(end_time))
    # print(threading.current_thread().name)
    
    # thread_id= threading._get_ident()

    # thread_file.write(str(thread_id))
    # thread_file.write("\n")
   
    # x= threading.current_thread()
    # for i in x:
    #     print(x)
    # thread_file.write("entered")
    # thread_file.write("\n")

    # proc = psutil.sProcess(thread_id)
    # proc= os.getpid()

    # thread_file.write(str(data))


    # cpu_perc=get_threads_cpu_percent(x)
    # thread_file.write(str(cpu_perc))
   
    # a=0
    # while a==0:
    #     thread_file.write("printing")
    #     thread_file.write("\n")
...
if __name__ == "__main__":
    ...
    controller=Thread(target = NCacheController('s1').main()).start()
    # load1,load5,load15= psutil.getloadavg()

    
    # cpu_usage= (load15/os.cpu_count())*100
    cpu_usage= psutil.cpu_percent(interval=None)
    thread_file.write(str(cpu_usage))


# if __name__ == "__main__":
#     # copy_to_cpu_function_call =0
#     # controller=Thread(target = NCacheController('s1').main()).start()
#     # my_function=Thread(target = func2).start()
...
#         cpu_perc= th.cpu_percent(interval=5)
